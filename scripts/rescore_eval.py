...
def main():
    ...
    for _, row in df.iterrows():
        ...
        for sp in spans:
            ...
            span_records.append({
                ...
                "asr_protocol": row.get("asr_protocol"),
    ...
    group_cols = ["pipeline"]
    span_group_cols = ["type", "pipeline"]
    if "asr_protocol" in out_df.columns:
        group_cols.append("asr_protocol")
        span_group_cols.append("asr_protocol")

    summary = {
        "rows": int(len(out_df)),
        "errors": int(out_df["has_error"].sum()),
        "auto_accuracy_by_pipeline": (
            out_df.dropna(subset=["auto_accuracy"]).groupby(group_cols)["auto_accuracy"].agg(["count", "mean"]).round(4).reset_index().to_dict("records")
        ),
        "per_type": (
            autoable_span.groupby(span_group_cols)["correct"].agg(["count", "mean"]).round(4).reset_index().to_dict("records")
        ),
        "human_only_counts": {
            f"{t}|{p}": int(v)
