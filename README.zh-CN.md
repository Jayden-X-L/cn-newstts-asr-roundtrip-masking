...
**一句话总结：** 当正确读法依赖上下文或行业规范时（体育比分、机型编号、技术单位、会员名称），ASR 回环可能对"人耳听来是错的"音频返回表面正确的转写。110 例 MiMo 定向审计中，**46 例确认被遮蔽、9 例被暴露、55 例 Raw TTS 无错**；上下文隔离在 18/46 中重新暴露局部错读证据；同批候选的 CosyVoice 审计确认 51 例被遮蔽。在 97 个确认遮蔽文件上，**Qwen3-ASR-1.7B 表面恢复 40/97，而 Paraformer-zh 仅 2/97**。结论：ASR 回环适合做筛查，不能作为独立的 ground truth。
...
- **2026 年 6 月 22 日：** 完整项目材料上传至 GitHub，并以 private 仓库保存，形成带时间戳的内部记录。
- **2026 年 7 月 16 日：** 相关中国专利申请正式递交，形成正式的优先权记录；该申请行为本身不等同于公开发表。
- **2026 年 7 月 22 日：** 扩展预印本提交至 arXiv，目前处于分类审核阶段；论文上线后，submission history 预计将保留该提交日期。
- **2026 年 7 月 26 日：** GitHub 仓库及支撑材料转为公开，构成首次明确的公开发布。
- **2026 年 7 月 27 日：** 归档数据集在 Zenodo 正式发布，DOI 为 [10.5281/zenodo.21454402](https://doi.org/10.5281/zenodo.21454402)（v1.0.0，CC BY 4.0），并已被 OpenAIRE 收录。
